...
export default function LionExOrdersPage() {
  ...
  useEffect(() => {
    ...
    async function loadOrders({ showLoading = false } = {}) {
      ...
      try {
        ...
        if ((orderFetchFailed || nextOrders.length === 0) && storeLookupKey) {
          ...
          if (Array.isArray(storedOrdersResponse?.data?.orders)) {
            nextOrders = storedOrdersResponse.data.orders;
          }
        }
        ...
        if (orderFetchFailed) {
          setOrderError(
            ordersResponse?.error || "Unable to load live Shopify orders. Showing saved orders if available."
          );
        }
      } catch (error) {
        if (!isMounted) return;
  ...
  return (
    <div className="lionex-orders-page">
      ...
      <footer className="lionex-orders-footer">
        <div>
